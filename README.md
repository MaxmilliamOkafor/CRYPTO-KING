...
4. Go to **gmgn.ai**, **pump.fun**, or **dextools.io**. A small **👑 crown button** sits in the bottom-right of the page — click it to open the CRYPTO-KING panel. It's a live, on-page assistant (not a separate tab):
   - **🔴 Live new launches (real-time, automatic):** the top of the panel runs a self-refreshing feed — the background worker pulls the newest coin launches from pump.fun's API and risk-scores each one every ~15s, no clicking. Rows show score + signal + age; tick **"hide high-risk"** to surface only lower-risk fresh launches to research. Works on **any** page (gmgn, pump.fun, dextools) because it pulls from an API, not the page's DOM. Tunables live in `config.ts → LIVE_FEED`. **For true snipe-speed cadence add a free Helius RPC key** (see below) and raise `LIVE_FEED.scanBudgetPerPoll` — the public RPC rate-limits how many coins can be deep-scanned per poll.
